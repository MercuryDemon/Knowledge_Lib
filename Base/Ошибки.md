...
Функции `errors.New` и `fmt.Errorf` генерируют ошибку, которая содержит сообщение - встроенный интерфейс. Они позволяют добавлять информацию к ошибкам, для этого нужен тип реализующий метод `Error`
...
```
Результат распоковки тоже может содержать метод `Unwrap`. Последовательность распоковок называется - цепочка ошибок.
