20-10-2025 18:50
Tags: #Go #lesson #DB 
# Взаимодействие Go с SQL

в Го нет драйвера SQL но есть интерфейс для работы с SQL базами данных.
...
//также подклбючаем sql дарайвер, например mySQL:
	_, "github.com/go-sql-driver/mysql"
// _ -чтобы не ругался, так как не используем оттуда ф-ции 
)
...
func main() {
	//подключение к базе через dsn(Data Source Name)
	// основные настройки к базе
	dsn := "root:password@tcp(localhost:3306)/coursera?"
	// указываем кодировку
	dsn += "&charset=utf8"
	...
	db, err := sql.Open("mysql", dsn) // инициализируем базу
	...
	r := mux.NewRouter()
	r.HandleFunc("/", handlers.List).Methods("GET") //пишем методы хэндлерам
...
никогда не подставлять значения в SQL запрос руками без Экранирования
- вместо этого используем placeholder - "?"
иначе будет SQL инъекция

### Zero-Links
- https://stepik.org/lesson/1133413/step/1?unit=1145037
