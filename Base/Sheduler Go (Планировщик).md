...
Для того чтобы масштабировать идею выполнения нескольких горутин на одном потоке, для процессоров с множеством ядер (потоков), было произведено шардирование (разбиение на кусочки) очереди всех горутин на разные локальные очереди закрепленные за каждым потоком.
...
- Имеет собственный стек (начинается с 2KB, растет по необходимости)
- Стоит гораздо дешевле потоков ОС (тысячи горутин могут работать на одном потоке)
- Хранит контекст выполнения (регистры, счетчик инструкций, стек)

#### M (Machine) — машина (поток ОС)

- Реальный поток операционной системы
- Управляется ОС, создается через системные вызовы
- Выполняет горутины на ядре процессора
- Максимальное количество ограничено (по умолчанию ~10,000)

#### P (Processor) — процессор (контекст планировщика)

- Логический процессор, выделяющий ресурсы для выполнения горутин
- Имеет локальную очередь горутин (LRQ — Local Run Queue)
- Количество **P** задается переменной `GOMAXPROCS` (по умолчанию = количество ядер CPU)
- Связывает M и G для выполнения

### Work Stealing

горутины созданные одним потоком, добавляются в локальную очередь этого же потока. Однако если у потока заканчиваются горутины в очереди, этот поток сам идет смотрит в очереди других и пиздит оттуда горутины себе в очередь.
...
принцип при котором если какая-то горутина обращается к ядру, тогда создается новый поток, который будет выполнять эту горутину. А прежний продолжает выполнять горутины из очереди.
...
- У тредов ОС(Machine) есть только локальные очереди(processor) планировщика
- у самого шедулера есть GRQ(Global Run Queue) глобальная очередь

Как горутины попадают в GRQ:
...
Для бесконечных циклов и CPU-Bound задач используем вытеснением горутины
