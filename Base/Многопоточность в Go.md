...
Разделение программы на потоки, для того чтобы разные функции выполнялись одновременно.
...
Вместо бесконечного цикла, для проверки закрытия канала можно использовать `range`
...
Горутина не блокируется каналом пока его буфер не заполнится.
...
Да, мы можем передавать каналы в качестве элементов в другой канал.
