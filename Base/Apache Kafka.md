...
- распределенная система (не один сервер, а несколько серверов)
...
- в брокере есть "ящики" для нескольких типов сообщений, он их туда принимает, а потребители сами забирают нужные им сообщения из брокера.
...
Брокеров может быть много, они между собоц общаются создавая Kafka Cluster
...
Данные считываются неупорядоченно из топика, но упорядоченно относительно партиции из которой читаем.
...
Может случится ситуация, когда один из брокеров отваливается. В таком случае все данные из расположенных на нем партиций топиков пропадают. Чтобы этого не происходило партиции реплицируются на другие брокеры.

**Set replication-factor (>1)** - надстройка при создании топиков. Сколько реплик каждой портиции создавать.

В случае репликации партиций, они разделяются на LEADER(главную) и FOLLOWER(догоняющую).
...
Commit сообщение состоит из полей:
...
Когда консюмер обрабатывает сообщение он отправляет commit сообщение в `__consumer_offsets` топик брокера.

Если этот консюмер отвалится после обработки, следущий консюмер сначала читает информацию из `__consumer_offsets` топика, а потом получает пачку сообщений с оффсетом.
...
## Exactly-once Kafka (2PC-commit)

2 Phase Commit - по сути транзакции в Кафка 
![[Pasted image 20260226215429.png]]

- применяется например при обогащении(хотим чтото дописать в сообщение) входящего сообщения и немедленной отправки в следующую очередь

1 Фаза:
1. Сервис, который хочет транзакционно получить обработать и отправить сообщение отправляет команду Begin -> Transaction Coordinator, передает transaction_id(хранится в transaction state)
2. Читает из топика
3. Предает Transaction Coordinator в transaction state в какую партицию будет писать и оффсеты
4. делает запись
5. Комитит оффсет
6. закрывает транзакцию

2 Фаза:
7. Координаор отправляет специальный тригер во все партиции, которые участвовали в транзакции. И партиции которые отработали в режиме рид-коммит получают доступ к этим новым данным.


Шаблон построения transaction_id
```
<prefix>.<group_id>.<topic>.<partition>
```
...
- https://www.youtube.com/watch?v=fGP0qoB78Nw
